...
def petstore_to_slate(apiname):
  apiname = apiname.lower()
  jsonfile = Path("./OAS/PetStore.json")
  slatefile = Path("./slate/petStore.md")
  ...
  print(slatecmd)
  if failure:
    print("RAML -> Slate formatted md file creation failed. Trying next inarray .")
...
def api_raml_to_slate(apiname):
  ...
  jsoncmd = "node ./oas-raml-converter/lib/bin/converter.js --from RAML --to OAS20 ./src/" +apiname+ "/" +apiname+ ".raml > ./OAS/"+apiname+".json"
  print(jsoncmd)
...
def copy_specs_to_build():
  ...
  try:
    ...
    print("Creation of the directory %s failed" % path + OSError)
  else:
